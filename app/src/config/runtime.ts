...
export interface ServerEntry {
  ...
  profile?: string
  // Deployer marking that this engine is reached over a high-latency WAN link
  // (relative to wherever the console runs). The app then starts light against
  // it instead of probing heavy follow= shapes that a WAN hop cannot finish
  // inside the transport timeout: the VM list never rides follow=statistics,
  // the hosts read skips its richest rung, and VM-centric polling floors at
  // the 30s infra cadence (see isActiveEngineWan in servers/registry.ts).
  // Purely a performance profile — omitting it costs nothing but the first
  // 30s probe per shape per 10-min window on such a link.
  wan?: boolean
}
...
function toServers(
  list:
    { name?: string; url?: string; fqdn?: string; profile?: string; wan?: boolean }[] | undefined,
): ServerEntry[] {
  ...
  for (const item of list) {
    ...
    if (item.wan === true) entry.wan = true
    out.push(entry)
  }
  return out
}
...
function resolve(): RuntimeConfig {
  ...
  const servers = toServers(parsed.success ? parsed.data?.servers?.list : undefined)
  // A multi-engine build that resolves ZERO servers is almost always a broken
  // config.js (404, ConfigMap edit gone wrong, parse failure), and the symptom
  // — no picker, sign-in 404ing against the console's own origin — points
  // nowhere near the cause. Warn so the state is diagnosable from a user's
  // console screenshot (PERMIT_CATALOG_FALLBACK precedent).
  if (multiEngineCapable() && servers.length === 0 && typeof window !== 'undefined') {
    console.warn(
      '[runtime-config] multi-engine build resolved no configured servers — ' +
        (injectedGlobal === undefined
          ? 'config.js is missing or did not run'
          : 'servers.list is absent, empty, or every entry was invalid') +
        '; the console will talk same-origin only.',
    )
  }

...
// session-scoped view memory for the folder selection (see the restore
// effect). The full key carries the active engine base (computed in the
// component — the base isn't final until after login): tag ids are
// engine-local, so a tab that logs into another engine must not restore this
// engine's folder onto it. usePruneGhostFolder would drop such a ghost
// anyway, but the memory shouldn't leak across engines in the first place.
const FOLDER_MEMORY_PREFIX = 'console-inventory-folder'
...
export function VmsAndTemplatesPage() {
  ...
  // A folder id this engine's tag list doesn't contain filters everything to
  // zero while the page still reads like the unfiltered root — drop it once
  // tags have loaded (see usePruneGhostFolder).
  usePruneGhostFolder(selectedFolderId, tags)
  ...
  const folderMemoryKey = `${FOLDER_MEMORY_PREFIX}:${getActiveBase()}`
  const navigate = useNavigate()
  const restoredFolder = useRef(false)
  useEffect(() => {
    if (!restoredFolder.current) {
      restoredFolder.current = true
      const stored = sessionStorage.getItem(folderMemoryKey)

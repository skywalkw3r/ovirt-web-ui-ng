...
export function LoginPage() {
  ...
  useEffect(() => {
    // !isAuthenticated: an authenticated visit to /login (bookmark, back
    // button) redirects away — snapping the picker under a LIVE session would
    // repoint the active base while a token for another engine is held
    // (setActiveBase also refuses that itself; this keeps the effect honest).
    if (!isAuthenticated && servers.length > 0 && !servers.some((s) => s.base === activeBase)) {
      setActiveBase(servers[0].base)
    }
  }, [isAuthenticated, servers, activeBase])

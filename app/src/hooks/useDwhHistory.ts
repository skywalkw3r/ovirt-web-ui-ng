...
// The entity id is substituted VERBATIM into panel SQL (grafana-query.ts
// substituteVars) and the host/cluster callers pass the raw route param — so
// gate on the engine GUID shape at this trust boundary. Anything that isn't a
// plain UUID never reaches the DWH query (the query stays disabled, which for
// a bogus route id is moot: the entity page itself 404s).
const ENGINE_GUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i

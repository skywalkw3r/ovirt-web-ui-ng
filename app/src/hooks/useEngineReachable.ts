...
export function useEngineReachable(): boolean {
  ...
  useEffect(() => {
    ...
    const unsubscribe = cache.subscribe((event) => {
      // Only react to a query SETTLING — event.action.type says what actually
      // happened ('success'/'error' terminate a fetch). Reading the query's
      // cumulative state.status here instead would miscount: a refetch start
      // ('fetch') or a retry attempt ('failed') on a query that still holds
      // old data reports status 'success' and would reset the counter while
      // the engine is down — and an already-errored query's every non-settling
      // event would count as a fresh failure.

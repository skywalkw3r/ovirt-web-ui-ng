import { z } from 'zod'
import { getActiveBase, getRequestBase } from '../servers/registry'
...
export async function revokeToken(token: string): Promise<boolean> {
  try {
    // getRequestBase: the revoke must reach the engine that ISSUED this token
    // — never wherever the picker points now (see servers/registry.ts).
    const response = await fetch(`${getRequestBase()}${SSO_LOGOUT_PATH}`, {
      method: 'POST',

...
describe('fetchWithFollowFallback', () => {
  ...
  it('degrades to the bare read on a transport timeout and remembers the denial', async () => {
    vi.spyOn(console, 'warn').mockImplementation(() => {})
    const follow = vi.fn().mockRejectedValue(new DOMException('timed out', 'TimeoutError'))
    await expect(fetchWithFollowFallback('k', follow, async () => 'bare')).resolves.toBe('bare')
    expect(isFollowDenied('k')).toBe(true)
    await expect(fetchWithFollowFallback('k', follow, async () => 'bare2')).resolves.toBe('bare2')
    expect(follow).toHaveBeenCalledTimes(1)
  ...
  it('propagates a caller abort without denying', async () => {
    const follow = vi.fn().mockRejectedValue(new DOMException('aborted', 'AbortError'))

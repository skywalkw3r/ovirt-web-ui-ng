...
export async function request(path: string, opts: RequestOptions = {}): Promise<unknown> {
  ...
  // getRequestBase, not getActiveBase: the bearer token below is only valid
  // on the engine that issued it, so a live session pins the base even if the
  // active pick has been repointed (see servers/registry.ts).
  const response = await fetch(`${getRequestBase()}${API_PATH}${path}`, {

...
import {
  fetchWithFollowFallback,
  isDegradableFollowError,
  isFollowDenied,
  markFollowDenied,
} from '../followDegrade'
...
export async function listHostsUsage(search?: string): Promise<Host[]> {
  ...
  // A deployer-marked WAN engine never even probes the rich rung: the nested
  // per-NIC stats shape is the hosts analog of the follow=statistics read
  // that outran the transport timeout on the WAN engines — start at the
  // protected statistics rung instead of paying a 30s probe per TTL cycle.
  if (!isActiveEngineWan() && !isFollowDenied(richKey)) {
    try {
      ...
      // isDegradableFollowError adds the timeout leg the old 5xx-only check
      // lacked — a WAN tick that outruns the transport cap steps down the
      // ladder instead of erroring the hosts pane.

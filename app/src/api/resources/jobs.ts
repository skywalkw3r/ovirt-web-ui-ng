...
export async function listJobs(opts: { max?: number } = {}): Promise<Job[]> {
  ...
  let data
  try {
    // Shared follow-degrade helper (5xx or timeout → bare read + sticky
    // denial): this poll rides in the always-mounted chrome every 10s, so the
    // denial memory matters — a persistent owner-follow fault must not cost
    // two requests per tick for the whole session. 404 propagates out of the
    // helper (it only swallows degrade conditions) into the collection-absent
    // handling below.
    data = await fetchWithFollowFallback(
      'jobs.list:owner',
      async () => JobListSchema.parse(await request(`/jobs?max=${max}&follow=owner`)),
      async () => JobListSchema.parse(await request(`/jobs?max=${max}`)),
    )
  } catch (error) {

...
// follow this" — and a transport timeout means it can't follow it in TIME (a
// WAN engine with hundreds of VMs spends >REQUEST_TIMEOUT_MS assembling
// follow=statistics while the bare list answers in seconds; observed against
// a WAN-distant engine). Degrade through fetchWithFollowFallback so the denial STICKS
...
export async function getVm(id: string): Promise<Vm> {
  ...
  for (const [index, follow] of follows.entries()) {
    ...
    try {
      ...
      lastError = error
      // isDegradableFollowError: 5xx, transport timeout (a WAN engine that
      // can't finish the followed read), or the 400-on-follow engine quirk —
      // all step down the ladder instead of failing the detail page.

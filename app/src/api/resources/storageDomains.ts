...
export async function listStorageDomains(
  ...
  // Shared follow-degrade helper: 5xx or transport timeout answers with the
  // bare list, and the denial memory stops every 30s tick re-probing a
  // persistently failing follow (this function predates the helper; migrated).
  const data = await fetchWithFollowFallback(
    'storagedomains.list:data_centers',
    async () =>
      StorageDomainListSchema.parse(

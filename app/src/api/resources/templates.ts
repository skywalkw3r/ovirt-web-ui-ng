...
// needs folder membership for every row); the shared follow-degrade helper
// (same as listVms) answers a 5xx OR transport timeout with the bare list and
// remembers the denial — the list matters more than the embedded tags, and a
// WAN engine whose followed read outruns the 30s cap degrades instead of
// erroring the Templates half of the inventory every poll tick.
export async function listTemplates(
  ...
  const read = async (o: { search?: string; follow?: string }) => {
    const data = TemplateListSchema.parse(
      await request(`/templates${queryFor(o)}`, { signal: opts.signal }),
    )
    return data.template ?? []
  }
  if (opts.follow === undefined) return read(opts)
  return fetchWithFollowFallback(
    `templates.list:${opts.follow}`,
    () => read(opts),
...
export async function getTemplate(id: string): Promise<Template> {
  // Per-template denial key (the Blank 500 is a per-entity condition — see
  // getHost's per-host rationale).
  return fetchWithFollowFallback(
    `templates.get:cluster:${id}`,
    async () =>
      TemplateSchema.parse(await request(`/templates/${encodeURIComponent(id)}?follow=cluster`)),
    async () => TemplateSchema.parse(await request(`/templates/${encodeURIComponent(id)}`)),
  )
}

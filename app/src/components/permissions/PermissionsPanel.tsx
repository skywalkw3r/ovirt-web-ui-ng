...
// Toolbar search: case-insensitive substring over the two free-text columns
// (resolved assignee, role name) — the label columns are two-value chips the
// All/Direct toggle and header sort already slice. An empty/whitespace query
// passes every row. Exported for the unit test.
export function permissionMatchesSearch(
  ...
  return (
    (assigneeName?.toLowerCase().includes(needle) ?? false) ||
    (roleName?.toLowerCase().includes(needle) ?? false)
  )
}
...
export function PermissionsPanel<T extends PermissionRow>({
  ...
  // Live toolbar filter (permissionMatchesSearch) — the list is already in
  // memory, so it applies per keystroke; SearchInput's commit gesture (Enter/
  // clear) has nothing extra to publish.
  ...
  const visibleRows =
    canFilterScope && scope === 'direct'
      ...
      : allRows
  // Search composes AFTER the scope toggle (both narrow, order irrelevant to
  // the result, but the no-match empty state keys off visibleRows vs
  // searchedRows to tell "nothing here" from "nothing matches").
  const searchedRows = visibleRows.filter((row) =>
    permissionMatchesSearch(search, principalOf(row, principalNames)?.name, row.role?.name),
  )
  ...
  const sortedRows = sortRows(searchedRows, sort, (permission, key) => {
  ...
  return (
    <>
        ...
        <ToolbarContent>
          <ToolbarItem>
            <SearchInput
              ...
              onChange={setSearch}
              onCommit={() => {}}
              ariaLabel={t('permissions.search')}
            />
          </ToolbarItem>
          {canFilterScope && (
            <ToolbarItem>
      ...
      {/* rows exist but the search filtered them all out — an empty query
          passes every row, so landing here implies a non-empty search */}
      {permissions.isSuccess && visibleRows.length > 0 && searchedRows.length === 0 && (
        <EmptyState titleText={t('permissions.empty.match.title')}>
          <EmptyStateBody>{t('permissions.empty.match.body')}</EmptyStateBody>
          <EmptyStateFooter>
            <EmptyStateActions>
      ...
      {permissions.isSuccess && searchedRows.length > 0 && (

...
// the shared SearchInput pulls PF pieces (Tooltip, Flex) the core mock above
// doesn't model; a bare input keeps the toolbar composition assertable
vi.mock('../list-toolbar/SearchInput', () => ({
  SearchInput: ({ ariaLabel, value }: { ariaLabel: string; value: string }) => (
    <input type="search" aria-label={ariaLabel} defaultValue={value} />
  ),
}))

const { PermissionsPanel, isDirectPermission, permissionMatchesSearch } =
  await import('./PermissionsPanel')
...
describe('PermissionsPanel', () => {
  ...
  it('offers the toolbar search box in every tier (read-only tabs filter too)', () => {
    state.caps = { tier: 'user', isAdmin: false, loaded: true }
    const html = render(ROWS)

    expect(html).toContain('aria-label="Search permissions"')
  })
...
describe('permissionMatchesSearch', () => {
  it('matches case-insensitively on assignee or role name', () => {
    expect(permissionMatchesSearch('ADMIN', 'admin@internal', 'SuperUser')).toBe(true)
    expect(permissionMatchesSearch('superuser', 'admin@internal', 'SuperUser')).toBe(true)
    expect(permissionMatchesSearch('operators', 'admin@internal', 'SuperUser')).toBe(false)
  })

  it('passes every row on an empty or whitespace query', () => {
    expect(permissionMatchesSearch('', 'anyone', 'AnyRole')).toBe(true)
    expect(permissionMatchesSearch('   ', undefined, undefined)).toBe(true)
  })

  it('never matches a row whose fields are both unresolved', () => {
    // merged rows can lack a principal and even a role name — they filter out
    // rather than crash
    expect(permissionMatchesSearch('admin', undefined, undefined)).toBe(false)
  })
})

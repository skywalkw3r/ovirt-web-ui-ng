...
describe('PaneHeader', () => {
  it('renders the name in a single h2 on the banner', () => {
    const html = renderToStaticMarkup(
      <PaneHeader icon={<svg />} name="node-01" kindId={'infra.kind.host' as never} />,
    )
  ...
  it('builds the meta line as kind then each fact, separator-joined', () => {
    const html = renderToStaticMarkup(
      <PaneHeader
        icon={<svg />}
        name="node-01"
        kindId={'infra.kind.host' as never}
        facts={['Default', 'node-01.lab.local']}
      />,
    )

    expect(text(html)).toContain('[infra.kind.host] · Default · node-01.lab.local')
    // the kind is the one bolded word on the line
    expect(html).toContain('<span class="app-pane-header__kind">[infra.kind.host]</span>')
  })
  ...
  it('drops facts the caller could not resolve, keeping the separators right', () => {
    ...
    expect(text(html)).toContain('[infra.kind.host] · Default · 10.0.0.4')
  ...
  it('renders the kind alone when there are no facts at all', () => {
    const html = renderToStaticMarkup(
      <PaneHeader icon={<svg />} name="dc-1" kindId={'infra.kind.datacenter' as never} />,
    )

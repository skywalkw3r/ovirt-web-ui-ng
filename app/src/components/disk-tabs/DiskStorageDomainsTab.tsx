...
export function DiskStorageDomainsTab({ disk }: { disk: Disk }) {
  const t = useT()
  const inlined = (disk.storage_domains?.storage_domain ?? []) as InlinedStorageDomain[]
  // client-side header sort; no default — the engine list order stands until a
  // header is clicked (see hooks/useColumnSort). Before the early return so
  // hook order stays stable.
  const { sort, thSort } = useColumnSort()
  // Live engines can leave the follow unresolved (bare { id } stubs — the tab
  // then showed raw GUIDs with dash type/status), so fill every missing field
  // from the cached storage-domains inventory; inlined values still win.
  const lookup = useStorageDomainLookup()
  const domains = inlined.map((domain) => {
    const full = lookup(domain.id)
    return {
      ...domain,
      name: domain.name ?? full?.name,
      type: domain.type ?? full?.type,
      status: domain.status ?? full?.status,
      external_status: domain.external_status ?? full?.external_status,
    }
  })

...
| --- | --- | --- |
| **In-memory token store** | `app/src/api/session.ts` (module singleton); read in `app/src/api/transport.ts:37` | The token lives only in JS memory for the tab's lifetime — no cookie, no `localStorage`, no URL — so there is no ambient credential to steal at rest and nothing survives a tab close. |
...
| **Memory-only secrets (no persistence)** | `session.ts`; console ticket in `NovncConsole.tsx:101` | Neither the bearer token nor the console ticket is logged, serialized to the URL, or written to the DOM/`localStorage`; only non-secret UI settings (theme, interval) hit `localStorage`. |
